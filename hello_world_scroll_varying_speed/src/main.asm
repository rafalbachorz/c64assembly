...
main
; Initialize the screen and set up the initial state for our animation
    jsr init_screen
...
; We start with 40 because the text will scroll left, and we want it to start just off the right edge of the screen
    ...
    lda speed_reload       ; LDS = LoaD Stack Pointer from memory, we use this to set the initial value for our speed counter   
    sta speed_counter      ; STA = STore Accumulator (A) into memory
...
; Main loop: wait for the next frame, render the text, update the position, and repeat
    ...
    dec speed_counter      ; DEC = DECrement memory by 1
    bne loop                ; BNE = Branch if Not Equal (Z flag clear)
    lda speed_reload       ; LDA = LoaD Accumulator (A) from memory
    sta speed_counter      ; STA = STore Accumulator (A) into memory
...
speed_reload
    !byte 3      ; 3 = slower, 4/5 = even slower
